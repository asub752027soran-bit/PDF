...
import { CustomAdsPanel } from './CustomAdsPanel';
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      {activeTab === 'monetization' && (
        <div className="space-y-6">
          {/* Sub-tab Navigation */}
          <div className="bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200/80 dark:border-slate-800 flex items-center gap-2">
            <button
              onClick={() => setMonetizationSubTab('custom_ads')}
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                monetizationSubTab === 'custom_ads'
                  ? 'bg-blue-600 text-white shadow-sm'
                  : 'bg-transparent text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
            >
              <Megaphone className="w-4 h-4" />
              <span>Self-Served Ads &amp; Direct Sponsorships</span>
              <span className="px-2 py-0.5 rounded-full bg-white/20 text-[10px] font-extrabold">
                {config.customAds?.length || 3} Active
              </span>
            </button>
          ...
          {/* Sub Tab 2: Google AdSense Master Configuration */}
          {monetizationSubTab === 'adsense' && (
